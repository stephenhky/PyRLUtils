...
class TransitionProbabilityFactory:
    ...
    def _generate_individual_reward_function(self) -> IndividualRewardFunction:
        ...
        def _individual_reward_function(
            ...
            if state_value not in self.transprobs.keys():
                return 0.

            if action_value not in self.transprobs[state_value].keys():
                return 0.
    ...
    def get_probability(
        ...
        if action_value not in self.transprobs[state_value]:
            return 0.

        probs = 0.
        for next_state_tuple in self.transprobs[state_value][action_value]:
