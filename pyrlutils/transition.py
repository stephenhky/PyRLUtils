...
def convert_openai_gymenv_to_transprob(envname):
    gymenv = gym.make(envname)
    P = gymenv.env.P

    factory = TransitionProbabilityFactory()
    for state_value, trans_dict in P.items():
        ...
        factory.add_state_transitions(state_value, new_trans_dict)
